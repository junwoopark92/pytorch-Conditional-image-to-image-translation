...
parser.add_argument('--print_freq', type=int, default=10, help='print image translation results')
...
for epoch in range(args.train_epoch):
    ...
    for (A, _), (B, _) in zip(train_loader_A, train_loader_B):
        ...
        print(
            '[%d][%d/%d] - time: %.2f, Disc A loss: %.3f, Disc B loss: %.3f, Gen loss: %.3f' % (
                (epoch + 1), iter, min(len(train_loader_A),len(train_loader_B)), per_epoch_time, torch.mean(torch.FloatTensor(Disc_A_losses)),
        ...
        if iter % args.print_freq == 0:
            with torch.no_grad():
                ...
                for (A, _), (B, _) in zip(test_loader_A, test_loader_B):
                    ...
                    path = os.path.join(args.dataset + '_results', 'img',
                                        str(iter + 1) + '_epoch_' + args.dataset + '_' + str(n + 1) + '.png')
